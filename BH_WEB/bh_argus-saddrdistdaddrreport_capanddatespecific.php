...
$query ="select saddr, COUNT(DISTINCT(daddr)) as distcount from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'ARGUSLOG') GROUP BY saddr ORDER BY distcount DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Src IP</th><th># of Distinct Dst IP</th><th>LINKS</th><th>GRAPHS</th></tr>";
...
echo '<a href="bh_ipstatsummary.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&ip='.$result_row["0"].'"title="Show the Stat Summary." target="_blank">SrcIP_Stats</a><br>';
