<?php
/*COMMENTS
FileName= bh_argus-saddrdaddrdportreport_capanddatespecific.php
QueryDescription= Use this query to see all Source, Destination IP, and Dest Ports used for a specific CapID, Date, and Time
ENDCOMMENTS*/
include ("bhvars.php");
...
$query ="select count(*) as count, saddr, daddr, dport from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'ARGUSLOG') GROUP BY saddr,daddr,dport ORDER BY count DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Count</th><th>Src IP</th><th>Dst IP</th><th>Dst Port</th><th>LINKS</th><th>GRAPHS</th></tr>";
...
<title>Bloodhound - Src & Dst IP & Dst Port Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>Src/Dst IP with Dst Port - Results </u></h3>";
echo "<b>Query Description:</b> This query will show a count of all Source and Destination IPs with their Dst Port used during this capture..<br /><br />";
echo "
...
echo "<h2>Src/Dst IP with Dst Port Report</h2>";
echo "<b>Query Description:</b> This query will show a summary of all Src/Dst IP and Dst Port during the time specified.<br /><br />";
echo ('
