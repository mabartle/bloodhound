...
$query ="select count(*) as count, saddr, dport from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'ARGUSLOG') GROUP BY saddr,dport ORDER BY count DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Count</th><th>Src IP</th><th>Dst Port</th><th>LINKS</th><th>GRAPHS</tr>";
...
echo '<a href="bh_argus-commreport-dportandsaddr_capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&dport='.$result_row["2"].'&saddr='.$result_row["1"].'"title="Show the Communication Report for the SrcIP and DPort." target="_blank">SRCIP_CR_DPORT</a><br />';
...
<title>Bloodhound - SrcIP-->DstPort Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>SrcIP --> DstPort Query - Results </u></h3>";
echo "<b>Query Description:</b> This query will show a count of all Source IPs and who they communicate to (DstPort) used during this capture..<br /><br />";
echo "
...
echo "<h2>SrcIP --> DstPort  Report</h2>";
echo "<b>Query Description:</b> This query will show a summary of all Source IPs and who they communicate to (DstPort) during the time specified.<br /><br />";
echo ('
