...
$query ="select count(*) as count, http_user_agent from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'URLLOG') GROUP BY http_user_agent ORDER BY count DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Count</th><th>HTTP_USER_AGENT</th><th>LINKS</th><th>GRAPHS</tr>";
while ($result_row = $result->fetchRow()){
echo "<tr><td>";
echo $result_row[0] .'</td><td>';
echo $result_row[1] .'</td><td>';
echo 'All DNS/URL Reports: ';
echo '<a href="bh_url-commreportuseragent_capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&useragent='.base64_encode($result_row["1"]).'"title="Show the Communication Report for this User Agent." target="_blank">CR_UserAgent</a>&nbsp';
echo '<a href="http://www.google.com/search?q='.$result_row["1"].'"title="Google Search for URL." target="_blank">Google Search HTTP_USER_AGENT</a></td><td>';
