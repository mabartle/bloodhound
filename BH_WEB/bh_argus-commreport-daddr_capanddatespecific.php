<?php
/*COMMENTS
FileName= bh_argus-commreport-saddr_capanddatespecific.php 
...
$query ="select count(*) as count, saddr, daddr, proto, dport from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (daddr = '$daddr') AND (log_type = 'ARGUSLOG') GROUP BY saddr, daddr, proto, dport ORDER BY count DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Count</th><th>Src IP</th><th>Dst IP</th><th>Proto</th><th>Dst Port</th><th>LINKS</th></tr>";
...
echo '<a href="http://whois.domaintools.com/'.$result_row["2"].'"title="WhoIs For DstIP" target="_blank">WHODSTIP</a></td></tr>';
}
echo ("</table>");
...
<title>Bloodhound - Destination Address Communication Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>Destination Address Communication Report Query - Results </u></h3>";
echo "<b>Query Description:</b> This query will show the Communication Report for the Destination Address used during this capture..<br /><br />";
echo "
...
<strong>Destination Address Search Criteria:</strong><br />
<li><strong>Destination Address = <u>$daddr</u></strong></li><br />
" ;
query_db($beg_date,$end_date,$beg_time,$end_time,$capid,$daddr);
}
else {
 /*Passing today's date into form*/
echo "<h2>Destination Address Communication Report</h2>";
echo "<b>Query Description:</b> This query will show the Communication Report for the Destination Address used during this capture..<br /><br />";
echo ('
...
<label>Destination Address to Search:
<input type="text" name="daddr" maxlength="12" size="12" id="daddr" value="%"/>
</label><br />
