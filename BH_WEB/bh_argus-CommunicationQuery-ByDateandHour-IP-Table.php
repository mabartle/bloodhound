...
ENDCOMMENTS*/
include ("bhvars.php");
function query_db($beg_date,$shour,$ehour,$sort,$bytes,$ipsearch,$port){
...
$query ="select count(*) as count, saddr, daddr, SUM(sbytes) as 'Total_SrcBytes', SUM(dbytes) as 'Total_DestBytes', SUM(spkts) as 'Total_SrcPkts', SUM(dpkts) as 'Total_DestPkts' from argus where (sdate = '$beg_date') AND (stime >= '$shour:00:00') AND (stime <= '$ehour:59:59') AND ((sbytes > '$bytes') OR (dbytes > '$bytes')) AND ((saddr LIKE '$ipsearch') OR (daddr LIKE '$ipsearch'))AND ((sport LIKE '$port') OR (dport LIKE '$port')) GROUP BY saddr, daddr ORDER BY $sort DESC";
...
" ;
query_db($beg_date,$shour,$ehour,$sort,$bytes,$ipsearch,$port);
}
else {
