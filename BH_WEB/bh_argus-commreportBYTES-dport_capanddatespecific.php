<?php
/*COMMENTS
FileName= bh_argus-commreportBYTES-dport_capanddatespecific.php
...
$query ="select count(*) as count, saddr, daddr, proto, dport, sum(bytes) as sumbytes, sum(pkts) as sumpkts from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (dport = '$dport') AND (log_type = 'ARGUSLOG') GROUP BY saddr, daddr, proto, dport ORDER BY sumbytes DESC";
...
<title>Bloodhound - Dst Port Communication Report by BYTES</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>Destination Port Communication Report (by BYTES) Query - Results </u></h3>";
echo "<b>Query Description:</b> This query will show the Communication Report for the Dst Port(s) by BYTES used during this capture..<br /><br />";
echo "
...
echo "<h2>Destination Port Communication Report (by BYTES)</h2>";
echo "<b>Query Description:</b> This query will show the Communication Report for the Dst Port(s) by BYTES  used during this capture..<br /><br />";
echo ('
