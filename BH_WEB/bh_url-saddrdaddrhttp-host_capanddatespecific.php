<?php
/*COMMENTS
FileName= bh_url-saddrdaddrhttp-host_capanddatespecific.php
QueryDescription= Use this query to see all SrcIP --> DstIP - HTTP_HOSTS url data for a specific CapID, Date, and Time
ENDCOMMENTS*/
include ("bhvars.php");
...
$query ="select count(*) as count, saddr, daddr, http_host from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'URLLOG') GROUP BY saddr, daddr, http_host ORDER BY count DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Count</th><th>SrcIP</th><th>daddr</th><th>HTTP_HOST</th><th>LINKS</th><th>GRAPHS</tr>";
...
<title>Bloodhound - SrcIP --> DstIP - HTTP Host Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>SrcIP --> DstIP - HTTP HOST Query - Results </u></h3>";
echo "<b>Query Description:</b> This query will show a count of all SrcIP --> DstIP - HTTP HOSTS from the URL data used during this capture..<br /><br />";
echo "
...
echo "<h2>SrcIP --> DstIP - HTTP HOST Report</h2>";
echo "<b>Query Description:</b> This query will show a summary of all SrcIP --> DstIP - HTTP HOST from the url data during the time specified.<br /><br />";
echo ('
