...
$query ="select count(*) as count, http_host from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'URLLOG') AND ((saddr = '$ip') OR (daddr = '$ip')) GROUP BY http_host ORDER BY count DESC";
