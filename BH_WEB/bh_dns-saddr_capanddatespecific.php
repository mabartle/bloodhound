...
$query ="select count(*) as count, saddr from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'DNSLOG') GROUP BY saddr ORDER BY count DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Count</th><th>Src IP</th><th>LINKS</th><th>GRAPHS</tr>";
...
echo '<a href="bh_argusurldns-saddrdaddr_httphost_dns-request-capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&requestorhost='.$result_row["1"].'"title="Show me all the argus, dns, and url data for this SrcIP." target="_blank">SrcIP</a></td><td>';
...
<title>Bloodhound - DNS SrcIP Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>DNS SrcIP Query - Results </u></h3>";
echo "<b>Query Description:</b> This query will show a count of all the SrcIPs querying the DNS Server from the DNS data used during this capture..<br /><br />";
echo "
...
echo "<h2>DNS SrcIP Report</h2>";
echo "<b>Query Description:</b> This query will show a summary of all the SrcIPs querying the DNS Server from the DNS data during the time specified.<br /><br />";
echo ('
