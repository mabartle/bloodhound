...
$query ="select sdate, stime, saddr, daddr, dport, proto, dns_request, dns_response, http_host, http_request_uri, alert_type, os_type, log_type from bh_table WHERE (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (stime >= '$beg_time')) AND ((sdate <= '$end_date') AND (stime <= '$end_time')) AND (saddr = '$requestorhost') OR (daddr = '$requestorhost') OR (http_host LIKE '%$requestorhost%') OR (dns_response LIKE '%$requestorhost%') ORDER BY sdate,stime ASC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Date</th><th>Time</th><th>SrcIP</th><th>DstIP</th><th>DstPort</th><th>Protocol</th><th>DNS_REQUEST</th><th>DNS_RESPONSE</th><th>HTTP_HOST</th><th>HTTP_URI</th><th>Alert Type</th><th>O/S</th><th>Log Type</th><th>LINKS</th></tr>";
...
echo 'Snort Reports: ';
echo '<a href="bh_snort-ipdetailreport_capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&addr='.$result_row["2"].'"title="Show me the IP Snort Detail Report for this Source IP." target="_blank">SnortSrcIPDR</a>&nbsp';
echo '<a href="bh_snort-ipdetailreport_capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&addr='.$result_row["3"].'"title="Show me the IP Snort Detail Report for this Destination IP." target="_blank">SnortDstIPDR</a></td></tr>';
}
echo ("</table>");
...
<title>Bloodhound - Argus, URL and DNS REQUEST Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>Argus, URL and DNS REQUEST Query - Results </u></h3>";
echo "<b>Query Description:</b> This query will show all Argus, DNS REQUESTS AND URL request for the host/request defined used during this capture.<br /><br />";
echo "
...
echo "<h2>Argus, URL and DNS REQUEST Report</h2>";
echo "<b>Query Description:</b> This query will show all ARGUS, DNS REQUESTS AND URL request for the host/request defined used during this capture.<br /><br />";
echo ('
