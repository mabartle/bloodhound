...
ENDCOMMENTS*/
include ("bhvars.php");
function query_db($beg_date,$end_date,$beg_time,$end_time,$capid,$useragent){
...
$query ="select count(*) as count, saddr, daddr, http_host, http_request_uri from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'URLLOG') AND (http_user_agent LIKE '$useragent') GROUP BY saddr, daddr, http_host, http_request_uri ORDER BY count DESC";
...
" ;
query_db($beg_date,$end_date,$beg_time,$end_time,$capid,$useragent);
}
else {
