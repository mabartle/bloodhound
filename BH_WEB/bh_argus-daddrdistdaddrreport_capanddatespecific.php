...
$query ="select daddr, COUNT(DISTINCT(saddr)) as distcount from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'ARGUSLOG') GROUP BY daddr ORDER BY distcount DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Dst IP</th><th># of Distinct Src IP</th><th>LINKS</th><th>GRAPHS</th></tr>";
while ($result_row = $result->fetchRow()){
echo "<tr><td>";
echo $result_row[0] .'</td><td>';
echo $result_row[1] .'</td><td>';
echo '<a href="bh_argus-commreport-daddr_capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&daddr='.$result_row["0"].'"title="Show the Communication Report for this SrcIP." target="_blank">CR_DSTIP</a>&nbsp';
echo '<a href="bh_argus-commreport-saddrordaddr_capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&saddr='.$result_row["0"].'"title="Show the Communication Report for this IP." target="_blank">CR_ALLFORDSTIP</a>&nbsp';
echo '<a href="http://whois.domaintools.com/'.$result_row["0"].'"title="WhoIs For DstIP" target="_blank">WHODSTIP</a>';
...
<title>Bloodhound - Src & Dst IP Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>Source & Destination Query - Results </u></h3>";
echo "<b>Query Description:</b> This query will show a count of all Source and Destination IPs used during this capture..<br /><br />";
echo "
...
echo "<h2>Source & Destination IP Report</h2>";
echo "<b>Query Description:</b> This query will show a summary of all Source & Destination IPs during the time specified.<br /><br />";
echo ('
