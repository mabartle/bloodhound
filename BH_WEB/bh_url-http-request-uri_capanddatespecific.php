<?php
/*COMMENTS
FileName= bh_url-http-request-uri_capanddatespecific.php
QueryDescription= Use this query to see all unique http-request-uri url data for a specific CapID, Date, and Time
ENDCOMMENTS*/
include ("bhvars.php");
...
$query ="select count(*) as count, http_request_uri from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'URLLOG') GROUP BY saddr, http_request_uri ORDER BY count DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Count</th><th>HTTP_REQUEST_URI</th><th>LINKS</th><th>GRAPHS</tr>";
...
<title>Bloodhound - HTTP_REQUEST_URI Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>SrcIP --> HTTP REQUEST URI Query - Results </u></h3>";
echo "<b>Query Description:</b> This query will show a count of all HTTP REQUEST URI from the URL data used during this capture..<br /><br />";
echo "
...
echo "<h2>HTTP REQUEST URI Report</h2>";
echo "<b>Query Description:</b> This query will show a summary of all HTTP REQUEST URI from the url data during the time specified.<br /><br />";
echo ('
