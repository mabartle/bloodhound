<?php
/*COMMENTS
FileName= bh_snort-srcipdetailreport-saddrdaddrd_capanddatespecific.php 
QueryDescription= Use this query to see a Detail Snort Report for a specific SrcIP - Count, saddr, daddr, class_type, alert_Type for a specific CapID, Date, and Time
ENDCOMMENTS*/
include ("bhvars.php");
function query_db($beg_date,$end_date,$beg_time,$end_time,$capid, $saddr){
...
$query ="select count(*) as count, saddr, daddr, class_type, alert_type from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'SNORTLOG') AND (saddr = '$saddr') GROUP BY saddr, daddr, class_type, alert_type ORDER BY count DESC";
...
echo 'Snort Reports: ';
echo '<a href="bh_snort-ipdetailreport_capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&addr='.$result_row["1"].'"title="Show me the IP Snort Detail Report for this SrcIP." target="_blank">SnortSrcIPDR</a><br />';
...
<title>Bloodhound - Snort SrcIP Detail Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>Snort SrcIP Detail Report Query - Results </u></h3>";
echo "<b>Query Description:</b> Use this query to see the SrcIP Snort Detail Report - Count, saddr, daddr, class_type, alert_Type for a specific CapID, Date, and Time.<br /><br />";
echo "
