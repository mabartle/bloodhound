<?php
/*COMMENTS
FileName= bh_url-commreportmethod_capanddatespecific.php
QueryDescription= Use this query to see all URL information for a specific HTTP METHOD ,CapID, Date, and Time
ENDCOMMENTS*/
include ("bhvars.php");
function query_db($beg_date,$end_date,$beg_time,$end_time,$capid,$httpmethod){
...
$query ="select count(*) as count, saddr, daddr, http_host, http_request_uri from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'URLLOG') AND (http_method LIKE '$httpmethod') GROUP BY saddr, daddr, http_host, http_request_uri ORDER BY count DESC";
...
echo '<a href="http://www.google.com/search?q='.$result_row["3"].'"title="Google Search for URL." target="_blank">Google Search HTTP_HOST</a></td></tr>';
}
echo ("</table>");
...
echo "<b>Query Description:</b> This query will show a count of all URLs from the URL data used during this capture for a specific HTTP METHOD.<br /><br />";
echo "
...
<strong>HTTP METHOD Search Criteria:</strong><br />
<li><strong>HTTP METHOD = <u>$httpmethod</u></strong></li><br />
" ;
query_db($beg_date,$end_date,$beg_time,$end_time,$capid,$httpmethod);
}
else {
...
<label>HTTP METHOD to Search:
<input type="text" name="httpmethod" maxlength="30" size="30" id="httpmethod" value="%"/>
</label><br />
