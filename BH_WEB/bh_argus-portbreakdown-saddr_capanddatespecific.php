...
$query ="select count(*) as count, dport from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (saddr = '$saddr') AND (log_type = 'ARGUSLOG') GROUP BY dport ORDER BY count DESC";
...
echo '<a href="bh_argus-commreport-dportandsaddr_capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&dport='.$result_row["1"].'&saddr='.$saddr.'"title="Show the Communication Report for the SrcIP and DPort." target="_blank">SRCIP_CR_DPORT</a></td><td>';
