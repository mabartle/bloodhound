<?php
/*COMMENTS
FileName= bh_snort-srcipcommreport-saddrdaddrd_capanddatespecific.php 
QueryDescription= Use this query to see the Communication Report - Count, saddr, daddr, class_type, alert_Type for a specific CapID, Date, and Time
ENDCOMMENTS*/
include ("bhvars.php");
...
$query ="select count(*) as count, saddr, COUNT(DISTINCT(daddr)), COUNT(DISTINCT(class_type)), COUNT(DISTINCT(alert_type)) from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'SNORTLOG') GROUP BY saddr ORDER BY count DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Count</th><th>Src IP</th><th>DistDstIP</th><th>DistClass</th><th>DistAlertType</th><th>LINKS</th></tr>";
...
echo '<a href="bh_argusurldns-saddrdaddr_httphost_dns-request-capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&requestorhost='.$result_row["1"].'"title="Show me all the argus, dns, and url data for this SrcIP." target="_blank">SrcIP</a><br />';
echo 'Snort Reports: ';
echo '<a href="bh_snort-ipdetailreport_capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&addr='.$result_row["1"].'"title="Show me the IP Snort Detail Report for this IP." target="_blank">SnortIPDR</a></td></tr>';
}
echo ("</table>");
...
<title>Bloodhound - Snort SrcIP Communication Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>Snort SrcIP Communication Report Query - Results </u></h3>";
echo "<b>Query Description:</b> Use this query to see the Communication Report by SrcIP - Count, saddr, daddr, class_type, alert_Type for a specific CapID, Date, and Time.<br /><br />";
echo "
...
echo "<h2>Snort SrcIP Communication Report</h2>";
echo "<b>Query Description:</b> Use this query to see the Communication Report by SrcIP - Count, saddr, daddr, class_type, alert_Type for a specific CapID, Date, and Time.<br /><br />";
echo ('
