<?php
/*COMMENTS
FileName= bh_argus-lowsrcanddstportreport_capanddatespecific.php
QueryDescription= Use this query to see all communications that have a SrcPort and Dst Port less than 1024 used for a specific CapID, Date, and Time
ENDCOMMENTS*/
include ("bhvars.php");
...
$query ="select count(*) as count, saddr, daddr, proto, sport, dport, SUM(bytes) as 'sumbytes', SUM(pkts) as 'sumpkts' from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'ARGUSLOG') AND ((bh_table.sport < '1024') AND (bh_table.dport < '1024')) AND (proto != '1') GROUP BY saddr, daddr, sport, dport ORDER BY count desc";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Count</th><th>Src IP</th><th>Dst IP</th><th>Protocol</th><th>SrcPort</th><th>DstPort</th><th>Sum Bytes</th><th>Sum Packets</th><th>LINKS</th><th>GRAPHS</th></tr>";
...
echo '<a href="http://whois.domaintools.com/'.$result_row["2"].'"title="WhoIs For DstIP" target="_blank">WHODSTIP</a></td><td>';
...
<title>Bloodhound - Low SrcPort and DstPort Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>Low Src and Dst Port Query - Results </u></h3>";
echo "<b>Query Description:</b> Use this query to see all communications that have a SrcPort and Dst Port less than 1024 used for a specific CapID, Date, and Time.<br /><br />";
echo "
...
echo "<h2>Low Src and Dst Port Query - Results</h2>";
echo "<b>Query Description:</b> Use this query to see all communications that have a SrcPort and Dst Port less than 1024 used for a specific CapID, Date, and Time.<br /><br />";
echo ('
