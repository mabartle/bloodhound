<?php
/*COMMENTS
FileName= bh_argus-commreport-srcport_capanddatespecific.php 
...
ENDCOMMENTS*/
include ("bhvars.php");
function query_db($beg_date,$end_date,$beg_time,$end_time,$capid, $sport){
...
$query ="select count(*) as count, saddr, daddr, proto, sport, dport from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (stime >= '$beg_time')) AND ((sdate <= '$end_date') AND (stime <= '$end_time')) AND (sport = '$sport') AND (log_type = 'ARGUSLOG') GROUP BY saddr, daddr, proto, sport, dport ORDER BY count DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Count</th><th>Src IP</th><th>Dst IP</th><th>Proto</th><th>Src Port</th><th>Dst Port</th><th>LINKS</th><th>GRAPHS</tr>";
...
<title>Bloodhound - Src Port Communication Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>Source Port Communication Report Query - Results </u></h3>";
echo "<b>Query Description:</b> This query will show the Communication Report for the Src Port(s) used during this capture..<br /><br />";
echo "
...
<strong>Src Port Search Criteria:</strong><br />
<li><strong>Source Port = <u>$sport</u></strong></li><br />
" ;
query_db($beg_date,$end_date,$beg_time,$end_time,$capid,$sport);
}
else {
 /*Passing today's date into form*/
echo "<h2>Source Port Communication Report</h2>";
echo "<b>Query Description:</b> This query will show the Communication Report for the Src Port(s) used during this capture..<br /><br />";
echo ('
...
<label>Source Port to Search:
<input type="text" name="sport" maxlength="6" size="6" id="sport" value="%"/>
</label><br />
