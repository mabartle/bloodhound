<?php
/*COMMENTS
FileName= bh_dns-dns_request_capanddatespecific.php
QueryDescription= Use this query to see all DNS Requests for a specific CapID, Date, and Time
ENDCOMMENTS*/
include ("bhvars.php");
...
$query ="select count(*) as count, dns_request from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'DNSLOG') GROUP BY dns_request ORDER BY count DESC";
...
echo '<a href="bh_urldns-httphost_dns-request-capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&requestorhost='.$result_row["1"].'"title="Show the DNS and URL requests for this Name for this capture." target="_blank">HTTP_HOST and DNS_REQUEST</a></td></tr>';
}
echo ("</table>");
...
<title>Bloodhound - DNS REQUEST Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>DNS REQUEST Query - Results </u></h3>";
echo "<b>Query Description:</b> This query will show a count of all DNS REQUESTS from the DNS data used during this capture..<br /><br />";
echo "
...
echo "<h2>DNS REQUEST Report</h2>";
echo "<b>Query Description:</b> This query will show a summary of all DNS REQUEST from the DNS data during the time specified.<br /><br />";
echo ('
