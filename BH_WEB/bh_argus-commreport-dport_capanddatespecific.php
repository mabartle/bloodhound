<?php
/*COMMENTS
FileName= bh_argus-commreport-dport_capanddatespecific.php 
...
$query ="select count(*) as count, saddr, daddr, proto, dport from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (dport = '$dport') AND (log_type = 'ARGUSLOG') GROUP BY saddr, daddr, proto, dport ORDER BY count DESC";
