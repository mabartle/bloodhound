<?php
/*COMMENTS
FileName= bh_argus-dportreportBYTES_capanddatespecific.php
QueryDescription= Use this query to see all Dest Ports by SUM BYTES used for a specific CapID, Date, and Time
ENDCOMMENTS*/
include ("bhvars.php");
...
$query ="select count(*) as count, dport, SUM(bytes) as 'sumbytes', SUM(pkts) as 'sumpkts' from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'ARGUSLOG') AND (proto != '1') GROUP BY dport ORDER BY sumbytes DESC";
...
echo '<a href="bh_argus-commreport-dport_capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&dport='.$result_row["1"].'"title="Show the Communication Report for this DstPort." target="_blank">CR_DPort</a>&nbsp</td><td>';
...
<title>Bloodhound - DestPort Report (by Sum Bytes)</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>Destination Port Query (by Bytes) - Results </u></h3>";
echo "<b>Query Description:</b> This query will show a count of all Destination Ports by Sum Bytes used during this capture. Without ICMP Protocol Information<br /><br />";
echo "
...
echo "<h2>Destination Port Report (by Bytes)</h2>";
echo "<b>Query Description:</b> This query will show a summary of all Destination Ports by Bytes during the time specified.<br /><br />";
echo ('
