...
$query =" select count(*) as count, sport from bh_table where (sport <= '1024') AND (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (stime >= '$beg_time')) AND ((sdate <= '$end_date') AND (stime <= '$end_time')) AND (log_type = 'ARGUS') GROUP BY sport ORDER BY count DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Count</th><th>Src Port</th><th>LINKS</th><th>GRAPHS</tr>";
...
<title>Bloodhound - SrcPort Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>Source Port Query - Results </u></h3>";
echo "<b>Query Description:</b> This query will show all Source Ports used during this capture..<br /><br />";
echo "
...
echo "<h2>Source Port Report</h2>";
echo "<b>Query Description:</b> This query will show a summary of all Source Ports during the time specified.<br /><br />";
echo ('
