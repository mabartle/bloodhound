...
QueryDescription= Use this query to see a Detail Snort Report for a specific IP - Count, saddr, daddr, class_type, alert_Type for a specific CapID, Date, and Time
ENDCOMMENTS*/
include ("bhvars.php");
function query_db($beg_date,$end_date,$beg_time,$end_time,$capid, $addr){
...
$query ="select count(*) as count, saddr, daddr, class_type, alert_type from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (log_type = 'SNORTLOG') AND ((saddr = '$addr') OR (daddr = '$addr')) GROUP BY daddr, saddr, class_type, alert_type ORDER BY count DESC";
...
<title>Bloodhound - Snort IP Detail Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>Snort IP Detail Report Query - Results </u></h3>";
echo "<b>Query Description:</b> Use this query to see the IP Snort Detail Report - Count, saddr, daddr, class_type, alert_Type for a specific CapID, Date, and Time.<br /><br />";
echo "
...
<li><strong>IP = <u>$addr</u></strong></li><br />
" ;
query_db($beg_date,$end_date,$beg_time,$end_time,$capid,$addr);
}
else {
 /*Passing today's date into form*/
echo "<h2>Snort IP Detail Report</h2>";
echo "<b>Query Description:</b> Use this query to see the IP Snort Detail Report - Count, saddr, daddr, class_type, alert_Type for a specific CapID, Date, and Time.<br /><br />";
echo ('
...
<input type="text" name="addr" maxlength="12" size="12" id="addr" value="%"/>
</label><br />
