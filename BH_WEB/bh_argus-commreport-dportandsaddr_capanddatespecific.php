<?php
/*COMMENTS
FileName= bh_argus-commreport-dportandsaddr_capanddatespecific.php 
...
ENDCOMMENTS*/
include ("bhvars.php");
function query_db($beg_date,$end_date,$beg_time,$end_time,$capid, $dport, $saddr){
...
$query ="select count(*) as count, saddr, daddr, proto, dport from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (dport = '$dport') AND (saddr = '$saddr') AND (log_type = 'ARGUSLOG') GROUP BY saddr, daddr, proto, dport ORDER BY count DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Count</th><th>Src IP</th><th>Dst IP</th><th>Proto</th><th>Dst Port</th><th>LINKS</th><th>GRAPHS</tr>";
...
<title>Bloodhound - Dst Port Communication Report</title>
</head>
<body>
...
if ($beg_date != NULL ){
echo "<h3><u>Destination Port Communication Report Query - Results </u></h3>";
echo "<b>Query Description:</b> This query will show the Communication Report for the Dst Port(s) used during this capture..<br /><br />";
echo "
...
<li><strong>Source IP = <u>$saddr</u></strong></li><br />
" ;
query_db($beg_date,$end_date,$beg_time,$end_time,$capid,$dport,$saddr);
}
else {
 /*Passing today's date into form*/
echo "<h2>Destination Port Communication Report</h2>";
echo "<b>Query Description:</b> This query will show the Communication Report for the Dst Port(s) used during this capture..<br /><br />";
echo ('
...
<label>SrcIP  to Search:
<input type="text" name="saddr" maxlength="15" size="15" id="saddr" value="%"/>
</label><br />
