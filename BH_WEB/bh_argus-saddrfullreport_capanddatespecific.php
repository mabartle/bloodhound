<?php
/*COMMENTS
FileName= bh_argus-saddrfullreport_capanddatespecific.php 
QueryDescription= Use this query to see the FULL Report - saddr, daddr, proto, sport, dport for a specific CapID, Date, Time, SRCIP
ENDCOMMENTS*/
include ("bhvars.php");
...
$query ="select sdate, stime, saddr, daddr, proto, sport, dport, bytes, sbytes, dbytes, pkts, spkts, dpkts from bh_table where (capid LIKE '$capid') AND ((sdate >= '$beg_date') AND (sdate <= '$end_date')) AND ((stime >= '$beg_time') AND (stime <= '$end_time')) AND (saddr = '$saddr') AND (log_type = 'ARGUSLOG') ORDER BY sdate DESC,stime DESC";
$result = $connection->query($query);
if (DB::isError($result)){
die ("Could not run the query: <br />". $query." ".DB::errorMessage($result));
}
echo ('<table border="1">');
echo "<tr><th>Date</th><th>Time</th><th>Src IP</th><th>Dst IP</th><th>Proto</th><th>Src Port</th><th>Dst Port</th><th>Bytes</th><th>SrcBytes</th><th>DstBytes</th><th>Pkts</th><th>SrcPkts</th><th>DstPkts</th><th>LINKS</th></tr>";
...
echo '<a href="bh_argusurldns-saddrdaddr_httphost_dns-request-capanddatespecific.php?beg_date='.$beg_date.'&end_date='.$end_date.'&beg_time='.$beg_time.'&end_time='.$end_time.'&capid='.$capid.'&requestorhost='.$result_row["3"].'"title="Show me all the argus, dns, and url data for this DstIP." target="_blank">DstIP</a></td></tr>';
}
echo ("</table>");
...
echo "<h2>Source Address Communication Report</h2>";
echo "<b>Query Description:</b> This query will show the Communication Report for the Source Address used during this capture..<br /><br />";
echo ('
